...
if (Test-Pm2AppExists -Name "cashflow") {
    for ($i = 1; $i -le 12; $i++) {
        ...
        Write-BootLog "Waiting cashflow :5000/:8443 ($i/12)..."
        Start-Sleep -Seconds 5
    }
    if ($cashflowOk) {
        ...
        Write-BootLog "WARNING: cashflow not healthy on :5000/:8443"
    }
} else {
...
if (-not $SkipFunnel) {
    if (Get-Command tailscale -ErrorAction SilentlyContinue) {
        Write-BootLog "Tailscale Funnel setup (3000 + cashflow :5000/:8443 if up)"
...
$cfShow = if (Get-Command Get-CashflowListenPort -ErrorAction SilentlyContinue) { Get-CashflowListenPort } else { 5000 }
Write-Host "金流: http://127.0.0.1:$cfShow/   (預設 5000；可用 CASHFLOW_PORT=8443)"
