-- Align RLS with granted capabilities so authorized staff don't hit permission errors
-- after the UI already shows the action (which previously caused repeated alert popups).
...
ALTER TABLE public.users ALTER COLUMN name TYPE VARCHAR(50);
