...
describe("sundayRest", () => {
  it("detects Sunday with local calendar (2026-07-19)", () => {
    ...
    expect(isFixedSundayRest("2026-07-19", false)).toBe(false);
  ...
  it("only allows X on Sunday for schedule edits when fixed rest is on", () => {
  ...
  it("forces Sunday changes back to X only when fixed rest is on", () => {
    const rows = [
      { date: "2026-07-19", shift: "A", userId: "u1" },
      { date: "2026-07-18", shift: "B", userId: "u1" },
    ];
    expect(enforceSundayRestOnChanges(rows)).toEqual([
      { date: "2026-07-19", shift: "X", userId: "u1" },
      { date: "2026-07-18", shift: "B", userId: "u1" },
    ]);
    expect(enforceSundayRestOnChanges(rows, false)).toEqual(rows);
  });
});
