...
會一併寫入集集 `store_config:jiji` 總店班別範本（若尚無目錄），並建立集集示範員工。

| 角色 | 帳號 | 密碼 | 登入分頁 | 店別 |
|------|------|------|----------|------|
| 竹山店長 | admin | admin123 | 店長/老闆登入 | 竹山 |
| 老闆 | boss | boss123 | 店長/老闆登入 | 可切店 |
| 集集店長 | jiji | jiji123 | 店長/老闆登入 | 集集 |
| 集集員工 | jiji01 | jiji01123 | 員工登入 | 集集 |
| 集集員工 | jiji02 | jiji02123 | 員工登入 | 集集 |

登入後 → **員工管理** 改真實姓名與密碼（班表只顯示姓名）。  
集集若尚未 seed，店長／老闆首次開啟店家設定時也會自動寫入總店範本（僅在尚無設定列時）。
